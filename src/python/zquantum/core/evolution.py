    ...
    if method == 'Trotter':
        ...
        for index_order in range(0, trotter_order): # iterate over Trotter orders
            for index_term in range(0, len(hamiltonian.terms)):
                pyquil_expitH_circuit = exponentiate(hamiltonian[index_term] * (time / trotter_order))
...
def time_evolution_derivatives(hamiltonian: pyquil.paulis.PauliSum, 
                                time: float, 
                                method: str = 'Trotter', 
    ...
    if method == 'Trotter':
        ...
        if trotter_order > 1:
            ...
            repeated_circuit = time_evolution(hamiltonian, time,
                                              method='Trotter', trotter_order=1)
            
            for position in range(0, trotter_order):
                for circuit_factor, different_circuit in zip(output_factors,
                                                             single_trotter_derivatives):

                    output_circuits.append(generate_circuit_sequence(
...
def generate_circuit_sequence(repeated_circuit: Circuit, 
                            different_circuit: Circuit, 
                            length: int, 
                            position: int) -> Circuit:
    """
