...
class Y(Gate):
    ...
    def __init__(self, qubit: int):
        matrix = sympy.Matrix(
            [[complex(0, 0), complex(0, -1)], [complex(1, 0), complex(0, 0)]]
        )
        super().__init__(matrix=matrix, qubits=(qubit,))
...
class Rx(Gate):
    ...
    def __init__(self, qubit: int, parameter: Union[float, sympy.Symbol] = "theta"):
        matrix = sympy.Matrix(
            [
                [
                    complex(sympy.cos(parameter / 2), 0),
                    complex(0, -1 * sympy.sin(parameter / 2)),
                ],
                [
                    complex(0, -1 * sympy.sin(parameter / 2)),
                    complex(sympy.cos(parameter / 2), 0),
