...
# It does not matter though, as we are only testing serialization and it contains
# variety of data to be serialized.
...
def test_orquestra_encoder_can_handle_optimization_result():
    ...
    deserialized_result = json.loads(
        json.dumps(result_to_serialize, cls=OrquestraEncoder)
    )
...
def test_orquestra_decoder_can_load_numpy_arrays():
    dict_of_arrays = {
        ...
        "array_3": {
            "real": [[1.5, 2.5], [0.5, 0.5]],
            "imag": [[0.5, -0.5], [1.0, 2.0]],
        },
    }
    ...
    for key in dict_of_arrays:
        assert np.array_equal(
            deserialized_object[key], expected_deserialized_object[key]
        )


@pytest.mark.parametrize(
    "value_estimate", [ValueEstimate(-2.5, 0.125), ValueEstimate(0.5)]
)
...
def test_orquestra_decoder_successfully_loads_history_entry_objects(history_entry):
    ...
    deserialized_history_entry = json.loads(
        serialized_history_entry, cls=OrquestraDecoder
    )
