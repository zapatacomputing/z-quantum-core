...
from . import types_copy
from .types_copy import (
    GenericAlias,
...
def _type_check(arg, msg, is_argument=True, module=None):
    ...
    if isinstance(arg, (type, TypeVar, ForwardRef, types_copy.UnionType, ParamSpec)):
        return arg
    if not callable(arg):
...
def _type_repr(obj):
    ...
    """
    if isinstance(obj, types_copy.GenericAlias):
        return repr(obj)
    ...
    if obj is ...:
        return "..."
    if isinstance(obj, types_copy.FunctionType):
        return obj.__name__
    return repr(obj)
...
def _collect_type_vars(types_, typevar_types=None):
    ...
    for t in types_:
        if isinstance(t, typevar_types) and t not in tvars:
            tvars.append(t)
        if isinstance(t, (_GenericAlias, GenericAlias, types_copy.UnionType)):
...
def _remove_dups_flatten(parameters):
    ...
    params = []
    for p in parameters:
        if isinstance(p, (_UnionGenericAlias, types_copy.UnionType)):
...
def _eval_type(t, globalns, localns, recursive_guard=frozenset()):
    ...
    if isinstance(t, (_GenericAlias, GenericAlias, types_copy.UnionType)):
        ...
        if isinstance(t, types_copy.UnionType):
        ...
        for arg in self.__args__:
            if isinstance(arg, self._typevar_types):
                if isinstance(arg, ParamSpec):
                    ...
                    arg = subst[arg]
            elif isinstance(arg, (_GenericAlias, GenericAlias, types_copy.UnionType)):
...
class _UnionGenericAlias(_GenericAlias, _root=True):
    ...
    def __eq__(self, other):
        if not isinstance(other, (_UnionGenericAlias, types_copy.UnionType)):
            return NotImplemented
...
class _ConcatenateGenericAlias(_GenericAlias, _root=True):
    def __init__(self, *args, **kwargs):
        super().__init__(
            *args, **kwargs, _typevar_types=(TypeVar, ParamSpec), _paramspec_tvars=True
        )
...
_allowed_types = (
    types_copy.FunctionType,
    types_copy.BuiltinFunctionType,
    types_copy.MethodType,
    types_copy.ModuleType,
    WrapperDescriptorType,
    MethodWrapperType,
    MethodDescriptorType,
)
...
def get_type_hints(obj, globalns=None, localns=None, include_extras=False):
    ...
    if isinstance(obj, type):
        ...
        for base in reversed(obj.__mro__):
            ...
            if isinstance(ann, types_copy.GetSetDescriptorType):
                ann = {}
    ...
    if globalns is None:
        if isinstance(obj, types_copy.ModuleType):
...
def get_origin(tp):
    ...
    if tp is Generic:
        return Generic
    if isinstance(tp, types_copy.UnionType):
        return types_copy.UnionType
    return None
