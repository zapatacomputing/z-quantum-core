...
class MockQuantumSimulator(QuantumSimulator):
    ...
    def run_circuit_and_measure(self, circuit, n_samples, **kwargs):
    ...
    def get_expectation_values(self, circuit, **kwargs):
        n_qubits = len(circuit.qubits)
        values = [random.random() for i in range(n_qubits)]
        return ExpectationValues(values)

    def get_exact_expectation_values(self, circuit, **kwargs):
